...
\label{sec:voxsel}
We chose $\Sexpr{Nmods}$ scans from $\Sexpr{Nmods}$ patients to perform exploratory data analysis, model fitting, and estimation of model cutoffs; these data will be referred to as the training data.  These scans were randomly selected.   
...
In the following section, we will use the DSI from the random forest model.  In the $\Sexpr{non_nmods}$ test scans, the median DSI was $\Sexpr{vals["GE", "median"]}$ for patients scanned in a GE scanner, $\Sexpr{vals["Philips", "median"]}$ for Philips, $\Sexpr{vals["Toshiba", "median"]}$ for Toshiba, and $\Sexpr{vals["Siemens", "median"]}$for Siemens (Supplemental Figure~\ref{fig:dsi_man}).  The patient with a failed segmentation was scanned using a GE scanner.  The Kruskal-Wallis test indicated a difference in medians ($\chi^2(\Sexpr{kt$parameter}) = \Sexpr{round(kt$statistic, 1)}
$, $p = \Sexpr{round(kt$p.value, 3)}$).  After Bonferroni correction, the difference of DSI of patients scanned with Siemens versus GE scanners was the only statistically significant comparison ($W = \Sexpr{round(sig$statistic, 1)}
